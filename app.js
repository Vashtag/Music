...
function transposeNote(note, n) { return midiToNote(Tone.Frequency(note).toMidi() + n); }
...
function buildScaleNotes(rootNote, scaleType, octaves = 2) {
  ...
  for (let o = 0; o < octaves; o++)
    SCALE_INTERVALS[scaleType].forEach(s => out.push(midiToNote(root + s + o*12)));
  return out;
}

// Integer 16th-note arithmetic — avoids floating-point rounding glitches
function beatsToTime(totalBeats) {
...
function audioBufferToWav(buf) {
  ...
  for (let i=0; i<len; i++)
    for (let c=0; c<nc; c++) {
      ...
      dv.setInt16(off, Math.round(s * 32767), true); off += 2;
    }
  return new Blob([ab], { type: 'audio/wav' });
}

// ─── Event builders ───────────────────────────────────────────────────────────
function computeArpEvents(prog, pattern, speed) {
  const sb = BL[speed] || 1, events = [];
  let startBeat = 0;
  for (const chord of prog) {
    const cb = chord.bars * 4;
    const root = Tone.Frequency(chord.root).toMidi();
    const notes = CHORD_INTERVALS[chord.quality].map(s => midiToNote(root + s + 12));
    let beat = 0, step = 0;
    while (beat < cb - sb * 0.4) {
      const pi = pattern[step % pattern.length];
      if (pi >= 0) events.push({ time: beatsToTime(startBeat+beat), note: notes[pi % notes.length], velocity: 0.28 + Math.random()*0.10 });
      beat += sb; step++;
    }
    startBeat += cb;
  }
...
function computePadEvents(prog) {
  const events = []; let sb = 0;
  for (const chord of prog) {
    const root = Tone.Frequency(chord.root).toMidi();
    const base = CHORD_INTERVALS[chord.quality].map(s => midiToNote(root+s));
    const high = CHORD_INTERVALS[chord.quality].slice(0,2).map(s => midiToNote(root+s+12));
    events.push({ time: beatsToTime(sb), notes:[...base,...high], bars: chord.bars });
    sb += chord.bars * 4;
  }
  return events;
}

function computeBassEvents(prog) {
  const events = []; let sb = 0;
  for (const chord of prog) {
    events.push({ time: beatsToTime(sb), note: midiToNote(Tone.Frequency(chord.root).toMidi()-12), duration:`${chord.bars}m` });
    sb += chord.bars * 4;
  }
  return events;
}

function generatePhrase(scale, len, startIdx) {
  const contours = [[1,1,-1,2,-1,1],[-1,1,2,-1,1,-2],[1,2,1,-2,-1,1],[2,-1,1,1,-2,1],[-1,-1,2,-1,2,-1],[1,-2,1,2,-1,-1]];
...
function computeMelodyEvents(prog, scaleRoot, scaleType, every, reg) {
  if (!every) return [];
  const scale = buildScaleNotes(scaleRoot, scaleType, 2);
  const baseIdx = Math.floor(scale.length * (reg===1 ? 0.6 : 0.35));
  const totalBars = prog.reduce((s,c)=>s+c.bars,0);
  const durs = ['4n','4n','4n.','2n'];
  const events = [];
  for (let bar=0; bar<totalBars; bar+=every) {
    if (Math.random()<0.3) continue;
    const phrase = generatePhrase(scale, 4+Math.floor(Math.random()*5), baseIdx+Math.floor(Math.random()*3)-1);
    let beat = bar*4 + randomBetween(0,1);
    for (const note of phrase) {
      if (beat >= totalBars*4) break;
      const dur = pickFrom(durs);
      events.push({ time: beatsToTime(beat), note, duration: dur, velocity: 0.20+Math.random()*0.18 });
      beat += BL[dur] || 1;
    }
  }
  return events;
...
let renderedBlob    = null;   // WAV blob — available immediately after render
let renderedLabel   = '';
let previewPlayer   = null;   // Tone.Player looping the rendered buffer
...
(function initBg() {
  ...
  function draw() {
    ...
    for (const p of particles) {
      ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle=`rgba(108,99,255,${p.a})`; ctx.fill();
      p.x+=p.vx; p.y+=p.vy;
...
document.getElementById('custom-minutes').addEventListener('input', e => {
  ...
  if (v > 0 && v <= 180) { durationMinutes = v; document.querySelectorAll('.dur-btn').forEach(b => b.classList.remove('active')); }
});

// ─── Engine ───────────────────────────────────────────────────────────────────
// Params are picked in the constructor so they're known before Tone.Offline() is called.
class AmbientEngine {
  constructor(moodKey) {
  ...
  _pick() {
    ...
    return {
      bpm, totalBars, loopSec,
      progression, scaleRoot,
      arpPattern:  pickFrom(cfg.arpPatterns),
      arpSpeed:    pickFrom(cfg.arpSpeeds),
      reverbWet:   randomBetween(cfg.reverbWetRange[0],  cfg.reverbWetRange[1]),
      reverbDecay: randomBetween(cfg.reverbDecayRange[0], cfg.reverbDecayRange[1]),
      filterFreq:  Math.round(randomBetween(cfg.filterFreqRange[0], cfg.filterFreqRange[1])),
      padAttack:   randomBetween(cfg.padAttack[0],  cfg.padAttack[1]),
      padRelease:  randomBetween(cfg.padRelease[0], cfg.padRelease[1]),
      melodyEvery: cfg.melodyEveryRange[1]===0 ? 0 : Math.round(randomBetween(cfg.melodyEveryRange[0], cfg.melodyEveryRange[1])),
      melodyReg:   Math.random() < 0.5 ? 0 : 1,
      label: `${rootName} · ${bpm} BPM`,
    };
  }

  // Works inside Tone.Offline() AND in normal context.
  async buildNodes() {
    const { p, cfg } = this;
    Tone.Transport.bpm.value     = p.bpm;
    Tone.Transport.timeSignature = [4, 4];

    const reverb  = new Tone.Reverb({ decay: p.reverbDecay, preDelay: 0.1, wet: p.reverbWet });
    await reverb.ready;
    const filter  = new Tone.Filter(p.filterFreq, 'lowpass', -12);
    ...
    const pad = new Tone.PolySynth(Tone.Synth, {
      ...
      envelope:   { attack: p.padAttack, decay: 1, sustain: 0.85, release: p.padRelease },
    });
    ...
    const padPart = new Tone.Part((time, ev) => {
      ...
      pad.releaseAll(time + Tone.Time(`${ev.bars}m`).toSeconds() - p.padRelease*0.5);
    }, computePadEvents(p.progression));
    padPart.loop = true; padPart.loopEnd = `${p.totalBars}m`; this.parts.push(padPart);

    // Arp
    const arp = new Tone.Synth({ oscillator:{type:'sine'}, envelope:{attack:0.015,decay:0.5,sustain:0.2,release:1.0} });
    arp.volume.value = -26; arp.connect(master); this.nodes.push(arp);
    const arpPart = new Tone.Part((time, ev) => {
      arp.triggerAttackRelease(ev.note, p.arpSpeed, time+(Math.random()-0.5)*0.01, ev.velocity);
    }, computeArpEvents(p.progression, p.arpPattern, p.arpSpeed));
    arpPart.loop = true; arpPart.loopEnd = `${p.totalBars}m`; this.parts.push(arpPart);

    // Bass
    const bass = new Tone.Synth({ oscillator:{type:'sine'}, envelope:{attack:p.padAttack*0.7,decay:0.5,sustain:0.8,release:p.padRelease} });
    bass.volume.value = -20; bass.connect(master); this.nodes.push(bass);
    const bassPart = new Tone.Part((time, ev) => {
      bass.triggerAttackRelease(ev.note, ev.duration, time, 0.55);
    }, computeBassEvents(p.progression));
    bassPart.loop = true; bassPart.loopEnd = `${p.totalBars}m`; this.parts.push(bassPart);

    // Melody
    if (p.melodyEvery > 0) {
      const mel = new Tone.Synth({ oscillator:{type: cfg.style==='lofi'?'triangle':'sine'}, envelope:{attack:cfg.style==='lofi'?0.05:0.7,decay:0.3,sustain:0.45,release:cfg.style==='lofi'?1.2:2.5} });
      mel.volume.value = -24; mel.connect(master); this.nodes.push(mel);
      const melEvs = computeMelodyEvents(p.progression, p.scaleRoot, cfg.scaleType, p.melodyEvery, p.melodyReg);
      if (melEvs.length) {
        const melPart = new Tone.Part((time, ev) => mel.triggerAttackRelease(ev.note, ev.duration, time, ev.velocity), melEvs);
        melPart.loop = true; melPart.loopEnd = `${p.totalBars}m`; this.parts.push(melPart);
      }
    }

    if (cfg.hasPerc) this._buildPerc(master);
    this._buildTexture(cfg.texture);
  ...
  start() { this.parts.forEach(p=>p.start(0)); Tone.Transport.start(0); }

  dispose() {
    try { Tone.Transport.stop(); Tone.Transport.loop=false; } catch(e){}
...
// ─── Offline Render ───────────────────────────────────────────────────────────
async function renderOffline(moodKey) {
  await Tone.start();
  const eng = new AmbientEngine(moodKey);
  const { loopSec, label } = eng.p;

  // Animate a fake progress bar while OfflineAudioContext works
  // (no real progress events available from Web Audio API)
  const estimatedMs = (loopSec / 8) * 1000; // assume ~8x real-time render speed
  const startT = Date.now();
  const fakeTimer = setInterval(() => {
    const pct = Math.min(((Date.now()-startT)/estimatedMs)*85, 85);
    progressBar.style.width = pct + '%';
  }, 80);

  const toneBuffer = await Tone.Offline(async () => {
    await eng.buildNodes();
    eng.start();
  }, loopSec);
  ...
  return { toneBuffer, wav, loopSec, label };
}

// ─── Preview Playback ─────────────────────────────────────────────────────────
function startPreview(toneBuffer, color) {
  if (previewPlayer)   { previewPlayer.stop();   previewPlayer.dispose();   previewPlayer   = null; }
  if (previewAnalyser) { previewAnalyser.dispose(); previewAnalyser = null; }

  previewAnalyser = new Tone.Analyser('waveform', 512);
  previewPlayer   = new Tone.Player(toneBuffer);
  previewPlayer.loop = true;
  previewPlayer.connect(previewAnalyser);
...
function stopPreview() {
  if (previewPlayer)   { try { previewPlayer.stop(); previewPlayer.dispose(); } catch(e){} previewPlayer = null; }
  if (previewAnalyser) { try { previewAnalyser.dispose(); } catch(e){} previewAnalyser = null; }
  isPlaying = false;
}
...
function startVisualizer(color) {
  ...
  function draw() {
    ...
    ctx.clearRect(0,0,W,H);
    const data = previewAnalyser ? previewAnalyser.getValue() : new Float32Array(512);
    ctx.beginPath(); ctx.strokeStyle = color; ctx.lineWidth = 1.5;
    const step = W / data.length;
    for (let i=0;i<data.length;i++) { const x=i*step, y=(data[i]*0.5+0.5)*H; i===0?ctx.moveTo(x,y):ctx.lineTo(x,y); }
    ctx.stroke();
  }
  draw();
}
...
generateBtn.addEventListener('click', async () => {
  ...
  generateBtn.querySelector('.btn-label').textContent = 'Rendering…';
  statusRow.style.display = 'block';
  progressBar.style.width = '0%';
  statusText.textContent  = 'Rendering audio offline — this takes a few seconds…';
  visSection.style.display = 'none';
  downloadBtn.style.display = 'none';

  try {
    const { toneBuffer, wav, loopSec, label } = await renderOffline(selectedMood);
    renderedBlob  = wav;
    renderedLabel = label;

    const loopStr = loopSec < 60
      ? `${Math.round(loopSec)}s loop`
      : `${(loopSec/60).toFixed(1)} min loop`;

    statusText.textContent    = `Done! Previewing looped · Download is ready.`;
    nowPlaying.textContent    = `${MOODS[selectedMood].label} · ${label} · ${loopStr}`;
    downloadBtn.style.display = 'inline-flex';

    const color = MOODS[selectedMood].color;
    startPreview(toneBuffer, color);
    visSection.style.display = 'block';
    startVisualizer(color);
...
playPauseBtn.addEventListener('click', () => {
  ...
  if (isPlaying) {
    previewPlayer.stop(); stopProgress();
    playPauseBtn.textContent = '▶ Resume';
  } else {
    previewPlayer.start(); startProgress((durationMinutes*60*1000)-(Date.now()-playbackStart));
...
function stopPlayback(finished) {
  stopProgress(); stopVisualizer(); stopPreview();
  isPlaying = false; isGenerating = false;

  generateBtn.classList.remove('generating');
  generateBtn.querySelector('.btn-icon').textContent  = '▶';
  generateBtn.querySelector('.btn-label').textContent = selectedMood ? `Generate "${MOODS[selectedMood].label}"` : 'Select a Mood to Generate';
  generateBtn.disabled = !selectedMood;
  playPauseBtn.textContent = '⏸ Pause';
  progressBar.style.width  = finished ? '100%' : progressBar.style.width;
  statusText.textContent   = renderedBlob ? 'Stopped. Download is still available below.' : '';
...
downloadBtn.addEventListener('click', () => {
  ...
  a.href     = url;
  a.download = `ambientforge-${selectedMood}.wav`;
  a.click();
